...
class ShopifyClient {
    ...
    public function call($method, $path, $params=array(), $suffix = '')
    {
        ...
        $response = $this->curlHttpApiRequest($method, $url, $query, $payload, $request_headers, $suffix);
        $response = json_decode($response, true);
    ...
    private function curlHttpApiRequest($method, $url, $query='', $payload='', $request_headers=array(), $suffix)
    {
        $url = $this->curlAppendQuery($url, $query, $suffix);
        $ch = curl_init($url);
    ...
    private function curlAppendQuery($url, $query, $suffix)
    {
        if (empty($query)) return $url;
        if (is_array($query)) return "$url?".http_build_query($query).$suffix;
        else return "$url?$query$suffix";
    }
