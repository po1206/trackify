...
";
    ...
    function ptag($store) {
        ...
        $product = "
            ...
            var pd_id = __st['rid'];
            var tfy_shop = " . json_encode($shop) . "

            var trackify_product = function() {
                _handle = '';
                " . $this->_product($shop) . "
                if (pageurl.lastIndexOf('?') > -1) {
                    _handle = pageurl.substring(pageurl.lastIndexOf('products/')+9, pageurl.lastIndexOf('?'));
                } else {
                    _handle = pageurl.substring(pageurl.lastIndexOf('products/')+9);
                }
                if (_handle != '') {
                    xmlhttp = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
                    xmlhttp.onreadystatechange = function () {
                        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                            ...
                            if (r.tags.length > 0) {
                                for (i = 0; i < r.tags.length; i++) {
                                    if (r.tags[i].indexOf('rr_track') >= 0) {
                    ...
                    xmlhttp.open('GET', '/products/' + _handle + '.js', true);
                    xmlhttp.send();
                    console.log(_handle);
                }
            }
