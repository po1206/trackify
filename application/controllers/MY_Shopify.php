...
';
    ...
    function set_timezone($shop) {
        ...
        $this->Trackify_DB->update_settings(array("timezone" => $timezone), array("store_name" => $shop["store_name"]));
    }
    ...
    function init_ocu($shop, $pixel_id) {
        $em = (isset($_GET['em'])) ? strtolower($_GET['em']) : '';
        $ph = (isset($_GET['ph'])) ? strtolower($_GET['ph']) : '';
        $fn = (isset($_GET['fn'])) ? strtolower($_GET['fn']) : '';
        $ln = (isset($_GET['ln'])) ? strtolower($_GET['ln']) : '';
        $ct = (isset($_GET['ct'])) ? strtolower($_GET['ct']) : '';
        $st = (isset($_GET['st'])) ? strtolower($_GET['st']) : '';
        $zp = (isset($_GET['zp'])) ? strtolower($_GET['zp']) : '';
...
";
                ...
                if ($params == "") {
                    $output = "fbq('addPixelId', '" . $pixel . "');
";
                } else {
                    $output = "fbq('addPixelId', '" . $pixel . "', {$params});
";
                }
            }
        }
    ...
    function global_pixels($shop, $page, $print = 1) {
        ...
        if (!empty($shop["ca"])) {
            if ($page == "checkout") {
                ...
                $output .= $this->init_ocu($shop, $shop["ca"]);
            } else {
        ...
        if (!empty($shop["ca_2nd"])) {
            if ($page == "checkout") {
                ...
                $output .= $this->init_ocu($shop, $shop["ca_2nd"]);
            } else {
...
";
    ...
    function ptag($store) {
        ...
        $checkout = "else if (typeof Shopify != 'undefined' && typeof Shopify.checkout != 'undefined' && typeof Shopify.Checkout.step != 'undefined' && Shopify.Checkout.step == 'thank_you')             
            {                
                ...
                if (Shopify.checkout['token'] != 'undefined' && getCookie(Shopify.checkout['token'] + updated_at) != '1') 
                {
                    ...
                    if (t < 300) 
                    {
                        ...
                        var source = '" . base_url() . "checkout/' + Shopify.shop + '/?pd_id=' + pd_id + '&vr_id=' + vr_id + '&total_price=' + total_price + '&customer=' + Shopify.checkout.customer_id + '&order=' + Shopify.checkout.order_id;
                        appendurl(source);
    ...
    function niche_tracking($shop, $pids, $ptype, $print = 1) {        
        ...
        try {
            ...
            foreach ($pids as $pid) {
                ...
                if ($ptype == "product" && $shop["conv_value"] == TRUE) $total_price = $price;
...
";
            ...
            //handle_exception($e);
            return array("rr_tag" => "", "total_price" => $price, "track" => $track);
        }        
        
        if ($print == 1) {
            echo $track;
        }
        return array("rr_tag" => implode(" ", $rr_tag), "total_price" => $price, "track" => $track);
    }
...
";
    ...
    function checkout($store) {
        ...
        $catalog_line = $cont_type = $pd_or_vr_id = '';
        if (!empty($shop["ca"])) {
...
";
        ...
        $pixel_date = new DateTime();
        $pixel_date->setTimezone(new DateTimeZone($shop["timezone"]));
        $pixel_date = $pixel_date->format("M j \a\\t g:i:s A");
        ...
        $log_data = array(
                ...
                "pixel_id" => $shop["ca"],
                "product_id" => serialize($pids),
                "order_id" => $_GET["order"],
                "pixel_date" => $pixel_date
            );
        $this->Trackify_DB->log($log_data);
    }
    ...
    function update_feeds($store = "") {
        ...
        echo json_encode(array("result" => 1, "time" => time() - $t, "id" => $channel["id"]));
        die();
    }
